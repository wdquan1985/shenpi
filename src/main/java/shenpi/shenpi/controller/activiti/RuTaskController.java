package shenpi.shenpi.controller.activiti;
...
import shenpi.shenpi.service.activiti.HiprocdefManager;
import shenpi.shenpi.service.activiti.RuprocdefManager;
import shenpi.shenpi.service.system.FhsmsManager;
import shenpi.shenpi.util.AppUtil;
import shenpi.shenpi.util.Const;
import shenpi.shenpi.util.DateUtil;
import shenpi.shenpi.util.ImageAnd64Binary;
import shenpi.shenpi.util.Jurisdiction;
import shenpi.shenpi.util.PageData;
import shenpi.shenpi.util.PathUtil;
import shenpi.shenpi.util.Tools;
...
public class RuTaskController extends AcBusinessController {
	...
	public ModelAndView handle() throws Exception{
		...
		//String Bruce = pd.getString("BRUCE");//审批人的姓名+审批意见

		String msg = pd.getString("msg");
		if("yes".equals(msg)){								//批准
			map.put("审批结果", "【批准】" + OPINION);
			//map.put("Bruce意见", Bruce);//审批结果
			...
			//map.put("Bruce意见", Bruce);//审批结果//审批结果
			setVariablesByTaskIdAsMap(taskId,map);			//设置流程变量
